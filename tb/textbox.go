...
func NewTextBox(styles [4]text.Style, size image.Point) *TextBox { // ktye: removed win argument
	b := &TextBox{
		size:  size,
		text:  rope.Empty(),
		style: styles[0],
...
func (b *TextBox) Click(pt image.Point, button int) (int, [2]int64) {
	...
	case b.button == 0 && button == 1 && b.mods[2]: // ktye: removed win
		button = 2

	case b.button == 0 && button == 1 && b.mods[3]: // ktye: removed win
		button = 3
